...
module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		copy: {
			...
			pattern: {
				files: [
					{
						src: ['l?b/**', 'package.json', '!l?b/gruntExample.{js,html}'],
						dest: 'dist/<%= pkg.name %>/'
					}
				]
			}
	...
	/* Grunt Option example */
	// Custom task - Increments the patch version in the build
	grunt.registerTask('version-update-cli', 'Increment the version in package.json', function () {

		var pkg = grunt.file.readJSON('package.json'),
			packageFile = 'dist/' + pkg.name + '/package.json',
			content = grunt.file.readJSON(packageFile);

		content.version = grunt.option('releaseVer');
		grunt.file.write(packageFile, JSON.stringify(content, null, 2));
